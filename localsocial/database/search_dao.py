...
def search_user_by_exact_field(current_user_id, field, value, limit):
	cursor = handled_execute(db_conn, """
		...
		(userId IN SELECT firstUserId FROM userFriends
			WHERE firstUserId = %(current_user)s AND secondUserId IN
				(SELECT firstUserId FROM userFriends
					WHERE secondUserId=%(current_user)s)
		) AS areFriends,
	...
	for user_row in user_rows:
		(user_id, first_name, last_name, nick_name,portrait,
			are_friends, is_request_sent, is_request_pending, is_following) = user_row
