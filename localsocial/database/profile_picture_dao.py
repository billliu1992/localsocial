...
def create_profile_picture(profile_pic_obj):
	cursor = handled_execute(db_conn, """
		INSERT INTO profilePictures 
			(uploadedPictureId, setDate, leftBound, topBound, width, height)
...
def get_profile_picture_by_picture_id(picture_id):
	cursor = handled_execute(db_conn, """
		SELECT profilePictureId, uploadedPictureId, setDate, leftBound, topBound, width, height FROM profilePictures
